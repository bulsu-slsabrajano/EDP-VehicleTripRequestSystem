...
public class UserPanel extends BorderPane {
    ...
    private BorderPane buildListPanel() {
        ...
        cmbRole.setOnAction(e -> loadUsers(cmbRole.getValue(), cmbStatus.getValue()));
        cmbStatus = FxUtil.styledCombo(FXCollections.observableArrayList("All","Active","Inactive"));
        cmbStatus.setValue("All"); 
        cmbStatus.setOnAction(e -> loadUsers(cmbRole.getValue(), cmbStatus.getValue()));

        Button btnRefresh = FxUtil.btnPrimary("Refresh");
        btnRefresh.setOnAction(e -> { cmbRole.setValue("All"); 
        cmbStatus.setValue("All"); 
        loadUsers("All","All"); });
        ...
        btnUpdate.setOnAction(e -> {
            Object[] row = table.getSelectionModel().getSelectedItem();
            if (row == null) { 
            	FxUtil.showInfo(this, "Select a user first!"); 
            	return; 
            }
    ...
    class ViewUserPanel extends VBox {
        ...
        ViewUserPanel(){
            ...
            lblLicKey.getStyleClass().add("form-label"); 
            lblDsKey.getStyleClass().add("form-label");
            lblLicKey.setVisible(false); 
            lbLic.setVisible(false); 
            lblDsKey.setVisible(false); 
        ...
        void load(int userId, String role){
            try{
                ...
                if(rs.next()){
                    lbId.setText(String.valueOf(userId));
                    lbFn.setText(rs.getString("first_name"));
                    lbMn.setText(nvl(rs.getString("middle_name"),"—")); 
                    lbLn.setText(rs.getString("last_name"));
                    lbUn.setText(rs.getString("username"));
                    lbEm.setText(rs.getString("email_address"));
                    lbAd.setText(nvl(rs.getString("address"),"—")); 
                    lbRl.setText(role);
                   
                    String st=rs.getString("user_status");
                    lbSt.setText(st);
                ...
                boolean has=false;
                while(rsC.next()){has=true; Label l=new Label("• "+rsC.getString("contact_number")); 
                l.getStyleClass().add("form-label"); 
                contactsBox.getChildren().add(l);}
                if(!has){Label l=new Label("No contact numbers on record."); l.setStyle("-fx-text-fill:#888;");
                contactsBox.getChildren().add(l);}
            }catch(Exception e){e.printStackTrace();}
        }
    ...
    class AddUserPanel extends VBox {
        ...
        AddUserPanel(){
            ...
            role.setValue("Admin"); lic.setVisible(false); 
            lblLicLbl.setVisible(false);
            role.setOnAction(e->{boolean d="Driver".equals(role.getValue());lic.setVisible(d);
            lblLicLbl.setVisible(d);});
            ...
            FxUtil.addFormRow(form,"First Name *:", fn,  y++);
            FxUtil.addFormRow(form,"Middle Name:", mn, y++);
            FxUtil.addFormRow(form,"Last Name *:",  ln,  y++); 
            FxUtil.addFormRow(form,"Address:",     addr,y++);
            FxUtil.addFormRow(form,"Email *:",      em,  y++); 
            FxUtil.addFormRow(form,"Username *:",  un,  y++);
            FxUtil.addFormRow(form,"Password *:",   pw,  y++); 
            FxUtil.addFormRow(form,"Role *:",      role,y++);
            form.add(lblLicLbl,0,y); form.add(lic,1,y);
           
            GridPane.setMargin(lblLicLbl,new Insets(6,10,6,10));
            GridPane.setMargin(lic,new Insets(6,10,6,10));
            Button btnAdd=FxUtil.btnPrimary("Add"), btnBack=FxUtil.btnOutlinePrimary("Back");
            btnAdd.setOnAction(e->save()); 
            btnBack.setOnAction(e->{clear();cards.show("LIST");
            loadUsers("All","All");});
        ...
        private void save(){
            ...
            if(first.isEmpty()){
            	FxUtil.setError(lblStatus,"First name required!");
            	return;
            }
            if(last.isEmpty()){
            	FxUtil.setError(lblStatus,"Last name required!");
            	return;
            }
            if(user.isEmpty()){
            	FxUtil.setError(lblStatus,"Username required!");
            	return;
            }
            if(p.isEmpty()){
            	FxUtil.setError(lblStatus,"Password required!");
            	return;
            }
            if(p.length()<8){
            	FxUtil.setError(lblStatus,"Password must be ≥8 chars!");
            	return;
            }
            if(email.isEmpty()){
            	FxUtil.setError(lblStatus,"Email required!");
            	return;
            }
            if(!EMAIL_PATTERN.matcher(email).matches()){
            	FxUtil.setError(lblStatus,"Invalid email format!");
            	return;
            }
            if("Driver".equals(r)&&!lic.getText().trim().isEmpty()){
                try{PreparedStatement c=conn.prepareStatement("SELECT COUNT(*) FROM Driver WHERE license_number=?");
                    c.setString(1,lic.getText().trim());
                    ResultSet rc=c.executeQuery();
                    if(rc.next()&&rc.getInt(1)>0){
                    	FxUtil.setError(lblStatus,"License number already exists!");
                    	return;
                    	}
                    }
                catch(Exception e){
            ...
            try{
                ...
                cu.setString(1,user);
                ResultSet ru=cu.executeQuery();
               
                if(ru.next()&&ru.getInt(1)>0){
                	FxUtil.setError(lblStatus,"Username already exists!");
                	return;
                }
                
                PreparedStatement ce=conn.prepareStatement("SELECT COUNT(*) FROM Users WHERE email_address=?");
                ce.setString(1,email);
                ResultSet re=ce.executeQuery();
                if(re.next()&&re.getInt(1)>0){
                	FxUtil.setError(lblStatus,"Email already exists!");
                	return;
                }
                ...
                ps.setString(2,mn.getText().trim().isEmpty()?null:mn.getText().trim());
                ps.setString(3,last); 
                ps.setString(4,addr.getText().trim().isEmpty()?null:addr.getText().trim());
                ...
                if(newId==-1){
                	FxUtil.setError(lblStatus,"Failed to get user ID!");
                	return;
                }
            ...
            switch(r){
                ...
                FxUtil.setSuccess(lblStatus,"User added successfully!");
                clear(); 
                loadUsers("All","All");
            }catch(Exception e){
            	FxUtil.setError(lblStatus,"Error: "+e.getMessage());e.printStackTrace();
            }
        }
        private void clear(){
        	fn.clear();
    ...
    class DeleteUserPanel extends VBox {
        ...
        DeleteUserPanel(){
            ...
            int y=0;
            grid.setAlignment(Pos.CENTER);
            FxUtil.addInfoRow(grid,"User ID:",lId,y++);
            FxUtil.addInfoRow(grid,"First Name:",lFn,y++);
            FxUtil.addInfoRow(grid,"Middle Name:",lMn,y++);
            FxUtil.addInfoRow(grid,"Last Name:",lLn,y++);
            FxUtil.addInfoRow(grid,"Username:",lUn,y++);
            FxUtil.addInfoRow(grid,"Email:",lEm,y++);
            FxUtil.addInfoRow(grid,"Address:",lAd,y++);
            FxUtil.addInfoRow(grid,"Role:",lRl,y++);
            FxUtil.addInfoRow(grid,"Account Status:",lSt,y);
            
            Button btnConfirm=FxUtil.btnDanger("Confirm Deactivate"), 
            		btnBack=FxUtil.btnOutlinePrimary("Back");
            
            btnConfirm.setOnAction(e->deactivate());
        ...
        void setDetails(int id,String fn,String mn,String ln,String un,String em,String ad,String role,String st){
            ...
            lFn.setText(fn);
            lMn.setText(mn!=null?mn:"—");
            lLn.setText(ln);
            lUn.setText(un);
            lEm.setText(em);
            lAd.setText(ad!=null?ad:"—");
            lRl.setText(role); 
            lSt.setText(st);
        ...
        private void deactivate(){
            if(!FxUtil.confirm(this,"Set this user's account to Inactive?","Confirm Deactivate"))
            	return;
            try{
            	...
            	FxUtil.setError(lblMsg,"Error deactivating user!");
            	e.printStackTrace();
            }
        }
        private Label vl(){
    ...
    class UpdateUserPanel extends VBox {
        private int uid; 
        private String curRole="", curStatus="", origLic="";
        ...
        UpdateUserPanel(){
            ...
            VBox card=new VBox(0); card.getStyleClass().add("card");
            card.setMaxWidth(580);
            Label title=new Label("Update User"); 
            title.getStyleClass().add("title-medium");
            title.setMaxWidth(Double.MAX_VALUE); 
            title.setAlignment(Pos.CENTER);
            ...
            btnBack.setOnAction(e->{lblStatus.setText(" ");cards.show("LIST");
            loadUsers("All","All");});
        ...
        void setData(int id,String first,String mid,String last,String user,String email,String addr2,String pwd,String r,String st){
            uid=id; 
            curRole=r;
            curStatus=st!=null?st:"Active";
            pwdChanged=false;
            
            fn.setText(nvl(first));
            mn.setText(nvl(mid));
            ln.setText(nvl(last));
            this.addr.setText(nvl(addr2)); 
            em.setText(nvl(email));
            un.setText(nvl(user));
            pw.setText(nvl(pwd));
            role.setValue(r);
            status.setValue(curStatus);
            
            if("Driver".equals(r)){
                try{PreparedStatement ps=conn.prepareStatement("SELECT license_number FROM Driver WHERE driver_id=?");
                    ps.setInt(1,id);
                    ResultSet rs=ps.executeQuery();
                    if(rs.next()){origLic=nvl(rs.getString("license_number"));
                    lic.setText(origLic);}}
                catch(Exception e){
                	e.printStackTrace();
                }
                ...
                lic.setVisible(true);
                lblLicLbl.setVisible(true);
            } else { lic.setVisible(false);
            	lblLicLbl.setVisible(false);
            	lic.clear(); origLic=""; 
            }
            Platform.runLater(()->pwdChanged=false);
        }
        private void update(){
            ...
            if(pwdChanged&&!np.isEmpty()&&np.length()<8){
            	FxUtil.setError(lblStatus,"Password must be ≥8 chars!");
            	return;
            }
            if("Driver".equals(nr)&&lic.getText().trim().isEmpty()){
            	FxUtil.setError(lblStatus,"License required for Driver!");
            	return;
            }
            if("Driver".equals(nr)){
            ...
            try{
                ...
                if(ru.next()&&ru.getInt(1)>0){
                	FxUtil.setError(lblStatus,"Username already taken!");
                	return;
                }
                ...
                if(re.next()&&re.getInt(1)>0){
                	FxUtil.setError(lblStatus,"Email already taken!");
                	return;
                }
                ...
                if(pwdChanged&&!np.trim().isEmpty()){
                    ...
                    ps.setString(1,fn.getText().trim());
                    ps.setString(2,mnv);
                    ps.setString(3,ln.getText().trim());
                    ps.setString(4,adv);
                    ps.setString(5,email);
                    ps.setString(6,user);
                    ps.setString(7,np.trim());
                    ps.setString(8,nr);
                    ps.setString(9,ns);
                    ps.setInt(10,uid);
                } else {
